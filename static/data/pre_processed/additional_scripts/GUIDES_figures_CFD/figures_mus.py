...
with open('../CFD_Scoring/G_scores_mus.p', 'rb') as inp:
  off_target_scores = pickle.load(inp)

# Grab list of ensgs
ensg_d = {}
with open('tf_guides_mus.csv', 'r') as tf_guides_file:
...
for ensg_id in ensg_list:
  form = '../GUIDES/CRISPR-Library-Designer/static/data/GRCm38_exons/' + ensg_id + '_*'
...
# Pickle results
with open('all_scores_mus.p', 'wb') as all_scores_f:
  pickle.dump(all_scores, all_scores_f)

with open('selected_scores_mus.p', 'wb') as selected_scores_f:
